...
export default function ClubBeneficios() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
            ...
            <div key={b.id} className="bg-gray-900 rounded-lg border border-gray-800 overflow-hidden">
              <Link to={`/club-beneficios/${b.id}`} className="block group">
                ...
                <div className="p-4 pb-0">
                  <h3 className="text-white font-semibold group-hover:text-club-yellow transition">{b.name}</h3>
                  {b.description && (
              ...
              {!user ? (
                <div className="px-4 pb-4 pt-3">
                  <Link to="/login" className="block w-full bg-gray-700 text-gray-400 font-semibold py-2 rounded text-center text-sm hover:bg-gray-600 transition">
                ...
                <div className="px-4 pb-4 pt-3">
                  <button
                    ...
                    disabled={(userData?.points || 0) < b.pointsRequired}
                    className="w-full bg-club-yellow text-black font-semibold py-2 rounded hover:bg-yellow-400 transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Canjear
                  </button>
                </div>
      ...
      {message && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4" onClick={() => setMessage('')}>
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-8 max-w-sm w-full text-center" onClick={(e) => e.stopPropagation()}>
            <div className="text-club-yellow text-5xl mb-3">ℹ️</div>
            <p className="text-gray-300 text-lg mb-6">{message}</p>
            <button onClick={() => setMessage('')} className="bg-club-yellow text-black font-semibold px-6 py-2 rounded hover:bg-yellow-400 transition w-full">
              Cerrar
            </button>
          </div>
        </div>
